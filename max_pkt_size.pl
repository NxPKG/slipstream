...
my $PAD = "^_"; # \r\n";
my $SCAN = 'NXPKG_MAXPKTSIZE';
my $BEGIN = "BEGIN_$SCAN=";
my $END = "END_$SCAN";
my $SIPURL = 'sip:nxpkg.pl;transport';
...
pcap_loop($pcap, 0, \&process_packet, "nxpkgpkt");
...
sub process_packet
{
  ...
  # look for NXPKG_MAXPKTSIZE (TCP MTU detection)
  ...
  else
  {
    ...
    # look for sip:nxpkg.pl;transport SIP REGISTER packet
...
sub sip_udp_respond
{
  ...
  my $rndfile = "/tmp/.nxpkg.rndout." . time() . rand();
  wf($rndfile, $resp);
...
sub addUser
{
  ...
  wf("/tmp/.nxpkg.pktsize.$id", encode_json(\%len));
}
